...
2. Current user's permission to change (using the contracts) values in table columns, to add rows and columns to tables. Permissions can be set using the *ContractConditions* function in *Permissions* fields of table columns and in the *Write permissions
 / Insert / Update / New column* field on the table editing page. The conditions specified in the * Update * field specify the rights to change all columns of the table in general, the conditions in the fields * Permissions * impose additional restrictions for each column separately.
